...
`instrument.py` (the Python client on your PC) speaks all three. Your E2 `press` command was already path one.
...
**Alarms** are data: `{block, key, op, threshold, action}` tested 20× per second on the same status; fire once on the rising edge; `notify` pops a message (a "toast") on every phone, `relay:<n>:on|off` acts on B4. Saved in `/alarms.json`.
...
6. **STEP (the 3D exchange file) from KiCad** (*File → Export → STEP*) — the template already contains the merged board.
...
Write its `SPEC.md` with one verifiable number; build it on a branch of its own; bring it as the optional fifth demo item.
