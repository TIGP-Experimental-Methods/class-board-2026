...
<span class="small">Flash says *No serial data received*? Hold BOOT, tap RST, release — the board comes back on a **new COM port**; flash again. The port disappears and reappears at every reset — normal. USB-C **data** cable in the connector marked *USB*. Fallback for a late flash failure only: the LED `press` button + counter (the tutor writes it); finish the measurement version in Homework 1.</span>

<span class="small">**Secrets never go into the chat.** WiFi passwords and tokens live in `firmware/include/secrets.h` (git-ignored); you type the password there yourself; `git status` before every commit.</span>
...
Or show the work: a video introducing the board · tips for working with AI or KiCad · a polished web page · a game that teaches something. Bring a real problem to Lecture 2; the tutor helps you write its `SPEC.md`. It can be the optional fifth item of your demo. **Just build something great.**
