...
Source of truth for v0.7 (rev B): `notes/2026-09-13-nmr-respec-proposal.md` and `notes/2026-09-13-v07-nmr-circuits.md`
in the course repo (`G:\My Drive\2. Presentations\2026\20260910-TIGP`); the older full specification is
`10-Class-Board-Design-Brief.md`, sections 4–9. Read them before touching the board.
...
- Rule areas with owners: `ZONE_BASE` (instructor), `ZONE_A` (inputs + NMR receiver), `ZONE_B` (outputs, timing,
  NMR transmitter), `ZONE_C` (power and switching; its power-entry group belongs to the instructor). Reference
  numbering encodes the owner: 1–99 base · 1xx inputs (A) · 2xx power entry and rails · 3xx analog outputs (B) ·
  4xx relays and isolated inputs (C) · 5xx DIO/TRIG/expander (B) · 7xx receiver and clocks (A) · 8xx DDS and power
  stage (B) · 9xx mixer and IF (A), coil switches and external power input (C).
...
- One AGND–GND join: the net-tie / 0 Ω link at the Section A / base boundary. Nothing else joins them.

## Order of work
1. **Inspect.** Open the schematic PDF and the PCB. List the sections, the fixed items and the rule areas. Do not move anything yet.
...
10. **The NMR analog rules** (circuits document §8, binding): no ground pour under the tank node; the crossed-diode
    limiter and its return within 5 mm of the RX SMA centre pin; `LO_I`, `LO_Q` and the 4× LO clock ≥ 5 mm from the
    RX SMA, the tank pads and the amplifier input, with a ground plane between them and the amplifier; the transmit
    output and its return routed as a close pair with a loop area under 1 cm² and never through the receiver pour;
    the two IF paths laid out as mirror images so I and Q have the same delay; the quadrature divider and the mixer
    referenced to AGND, with only the LO clock crossing the boundary, at the tie point.
11. **Stop and summarise** in this order — Blocker / High / Medium / Low — each with net, location, mechanism, proposed fix and
