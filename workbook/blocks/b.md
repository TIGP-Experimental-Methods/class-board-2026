...
- **The power stage:** **OPA564** (1.5 A, 17 MHz) on a single supply from the external power input, biased at mid-rail, with its **enable pin used as the transmit gate** (`TX_EN` = GPIO 40), a current flag and a thermal flag read back by the firmware, and its thermal pad soldered to the ground pour with a via array. AC-coupled out through the link to the TX SMA on the front panel and to the coil screw terminal beside it.
- **The digital lines:** SN74AHCT541 octal buffer at +5V (3.3 V in, TTL out) for DIO1–8; 74HCT125 quad buffer for the two fast outputs (GPIO 18, 21); **SN74LVC1T45** level translator for TRIG (A side +3V3 = GPIO 38, B side +5V → 100 Ω → SMA, direction from GPIO 39; BAV99 clamp on the B side); 100 nF at every supply pin. The buffered lines do not end on a terminal of the main board any more: they run through the **board-to-panel link** to the front panel, where TTL1–8 land on a ten-way screw-terminal strip (eight signals and two GND positions) and the two fast outputs get their own SMAs.
- **The I²C expander:** a **TCA9535** on the I²C bus. It drives the eight DIO lines here and the seven **MODULE OUT** lines that cross to Section C's module header on the front panel, which is how the board keeps enough direct microcontroller pins for the NMR signals. Anything timing-critical — the fast outputs, TRIG, the transmit gate — stays on a direct pin.
...
The TTL and fast-output terminals are not on your sheet: those lines leave your section through the link to the front panel, which is Section C. `hardware/docs/student-deletions.md` is the authority for this list and says where each part sits on the PDF. Your **one part from the JLCPCB parts library** (JLCPCB is the factory that makes and assembles our boards; its part numbers look like C12345): the **AD9834BRUZ, C116589**, TSSOP-20 — the synthesizer U801 itself.

**Your zone on the PCB** (the outlined region of the physical board that is yours to route)**.** `ZONE_B`. The DAC on the socket side and the op-amp next to the SMAs, feedback resistors tight to the op-amp; the buffers near the socket end and their outputs gathered at the link pins that carry them to the panel, with the fast nets short and a ground return beside them; the synthesizer, its filter and the power stage in a line towards the link pins that carry TX. **The transmit current is the enemy of Section A:** route the transmit output and its return as a close pair and keep that loop under a square centimetre, well away from the receiver's ground.
