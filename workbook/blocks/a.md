...
- **The tuned front end:** the RX input — the RX SMA is on the front panel and reaches your section through the board-to-panel link — two 1N4148W diodes back to back where it enters the board (the crossed-diode limiter that clips transmitter leakage at ±0.7 V), the tank capacitors (1.2 nF + trim pads; the coil and this capacitor resonate at 89.4 kHz), 100 Ω series, a 1 MΩ DC return.
...
**Your zone on the PCB** (the outlined region of the physical board that is yours to route)**.** `ZONE_A`. The eight input networks in a row in AI order, close to the link pins that carry them up to the panel's SMAs, the ADC in the middle on AGND copper; the receiver in its own corner with the tank pads tight to the link pins that bring RX down from the panel and **no ground pour under the tank node** (copper there adds capacitance and detunes the resonance). Rules: nothing digital under the analog inputs; the local-oscillator lines (LO_I, LO_Q and the 336 kHz clock) stay at least 5 mm away from the RX link pins, the tank and the amplifier input; SPI enters from the socket side only; no tracks on either inner layer (both are unbroken ground planes); stay inside the zone.
