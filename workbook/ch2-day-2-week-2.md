...
- **(d) Isolation — Section C.** A relay's coil and contacts share no copper; an optocoupler's LED and transistor share no copper; creepage is the distance that keeps it so. The relay contacts are built to switch mains, so their creepage is a mains distance: 5 mm to everything else, on every layer.
...
ERC 0 / DRC 0 · `LCSC` field on every part · values match the PDF · decoupling next to the pins · no tracks on either inner layer · nothing outside your rule area · pin 1 marked on the silkscreen · ground pour joined · Section C creepage ≥ 2.5 mm at the isolated inputs · Section C MAINS nets: 5 mm to everything on every layer, top layer only, no vias, tracks ≥ 3 mm, the rating text on the silkscreen · high-current traces wide, no neck-downs · a silkscreen label on every connector · CI green.
