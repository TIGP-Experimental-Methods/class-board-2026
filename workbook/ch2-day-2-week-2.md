...
**Fri 2026-09-18, 14:20–16:20 + Homework 2, gated by the class PCB (printed circuit board) order on Mon 09-28.** Today you meet every IC on the board, learn the only six KiCad (the free program we draw the schematic and lay out the printed circuit board in) operations you need, place the parts missing from your sheet, and start routing (drawing the copper tracks between parts) your zone (the outlined region of the board that is yours). This week you finish it, fetch one part from LCSC (the parts catalogue of JLCPCB, the factory that makes and assembles our boards), open your pull request (PR — a request to merge your branch into the shared project; someone reviews it first, comments, and approves), and review a classmate's.

**Prerequisites:** HW1 done (E2 merged — its changes brought into the main line — two answers, KiCad 10 installed, the project opened once (the library is in `hardware/lib/`), `SPEC.md`). Bring your laptop; the schematic PDF is on the course site. Optional background: [chapter A](chA-electronics-from-zero.md) (self-study, before today if you have never seen a schematic).

**The hardware model.** The MCU (the microcontroller) stays on the dev board; the **class board is a carrier** it plugs into: the analog front end done properly, isolation, buffers, robust connectors. One design, five units, one order; **JLCPCB assembles everything, SMD and through-hole**. You solder nothing. The dev board alone keeps running the app as the permanent fallback.

---

## Part A — In class

### A.1 Electronics essentials, on our schematic
...
### A.2 Read your block, again
`/tutor L2`: two harder questions from your block page (*which pins carry the fastest signal; where does your ground join*). Then meet the neighbour who will review your PR (ring: A→B→C→D→E→A).

### A.3 KiCad live — the only six operations (watch)
On the projector, in the base sheet: **place** a 100 nF + 10 µF pair and wire it → **annotate** → *Tools → Update PCB from Schematic* → **drag** the two footprints (the copper patterns the parts are soldered onto) next to the pin → **route** three tracks (`X`, click, click) → **DRC** (`Inspect → DRC`; the design rule check — zero errors means it passes), then the **3D view** (`Alt+3`). V4 and V5 replay the same steps.

### A.4 Exercise E6 — schematic gaps
- [ ] `git switch -c b<N>-<name>` (a new branch: your own line of work inside the repository); open `hardware/class-board.kicad_pro`; open your sheet `student/b<N>_gapped.kicad_sch`.
...
### A.6 Exercise E7 (start) — route your zone
...
### A.7 PR + peer review, live (watch)
The instructor opens a PR from a student branch, requests the ring reviewer (the classmate who reviews your pull request: A reviews B, B reviews C, …, E reviews A), writes one comment against the checklist, shows *Request changes* vs *Approve*.
...
- [ ] **E7 Finish routing.** DRC **0 errors, 0 unrouted**; the ground pour joined (`B` refills zones); a 3D screenshot to `docs/students/<name>/zone-3d.png`. B1/B4/B5 owners have replicated channels: route one, copy the pattern. Watch V5 *Route, DRC, PR, review* first.
...
- [ ] **E9 Open your PR.** `git add hardware/` (your sheet, the PCB, the library) → commit → push (upload your commits to GitHub, the website where the repository is stored) → `gh pr create` with the 3D screenshot in the description → *Reviewers:* your ring reviewer. CI (continuous integration — the automatic build GitHub runs on every pull request) must be green.
...
**Mon 09-28 (instructor):** paste-merge the five zones (select everything inside `ZONE_B<N>` on your branch → *Paste Special → in place* on `main`, the main branch everyone builds on), full DRC, Gerbers/BOM/CPL (the manufacturing files, the parts list and the placement list), JLC quote with assembly, **order**. The PCB file is never text-merged; schematic sheets are separate files and merge in git (the tool that records the history of the files).
