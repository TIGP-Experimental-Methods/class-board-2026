...
Rules are data, not code: `{block, key, op, threshold, action}`. Twenty times a second the engine tests every rule against the same status the app sees, fires once on the rising edge, runs the action (`notify` → toast on every phone; `relay:<n>:on|off` → the switching driver in Section C) and broadcasts an `alarm` message. Rules persist in `/alarms.json` on the board. Every section adds one rule — the demonstration's "alarm → relay → notification" step.
