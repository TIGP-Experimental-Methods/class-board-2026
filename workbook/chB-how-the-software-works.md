...
The firmware is flashed over USB once; afterwards it can update itself over WiFi (ArduinoOTA). The flash holds the program, a second slot for OTA, and the LittleFS partition with the web app (`pio run -t uploadfs`). Two uploads, two things: **code** (`upload`) and **web app** (`uploadfs`).
