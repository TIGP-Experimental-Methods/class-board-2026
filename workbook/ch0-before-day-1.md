...
In Workshop 1 you publish your projects on the public web, and **you may need both hosts — have both accounts ready.** A static page (the project website; an app that runs entirely in the browser) goes on GitHub Pages (any public repository becomes a website — nothing to install, your GitHub account does it). Anything with a backend, a bot webhook or a secret — a LINE or Telegram bot, an API key, a scheduled task — goes on Cloudflare Pages + Workers (a free service that turns a repository into a public web page and runs server-side code). Sign up at https://dash.cloudflare.com/sign-up (free plan; verify the e-mail). Then, once: *Workers & Pages → Create → Pages → Connect to Git* → authorise Cloudflare for your GitHub account — and stop there; nothing else is created before class.
✔ Signed in to the dashboard; the *Connect to Git* screen lists your GitHub account. (If sign-up fails, Workshop 1 can start on GitHub Pages alone — but sort the Cloudflare account out before you build anything with a backend.)
