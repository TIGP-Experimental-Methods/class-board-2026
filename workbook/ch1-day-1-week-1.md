...
5. **Flash** (from the class repository root; one command per line):
   ...
   ```
   Your board was flashed by the instructor, so both commands reset it by themselves — no buttons. Two things are normal: **the board's USB port disappears and reappears at every reset** (the USB-to-serial converter is inside the chip), so a serial monitor has to reconnect after every flash and the first boot lines may be missing; and if `upload` ends with **"No serial data received"**, hold **BOOT**, tap **RST**, release BOOT — the board comes back on a **new COM port** (ours went COM6 → COM10), so run `pio device list` again and flash to that port (`--upload-port COMx` if PlatformIO picks the wrong one). No port at all: a data cable, the *USB* connector, then the same BOOT+RST.
