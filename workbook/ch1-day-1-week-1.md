...
**Repository.** Your project gets its own repository next to `class-board-2026` in your course folder. In VS Code: create a folder, open it (*File → Open Folder…*), *Source Control → Initialize Repository*, then *Publish Branch* — choose public or private (your choice; Cloudflare Pages deploys private repositories, GitHub Pages needs a public one). Git is used from VS Code's Source Control panel throughout the course; when a terminal command is unavoidable, Claude runs it and tells you what it did.
