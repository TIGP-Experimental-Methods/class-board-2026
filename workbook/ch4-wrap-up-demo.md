...
---
**Tutor notes (`/tutor WRAP`, `/tutor DEMO`).** WRAP: environment `esp32s3`, never `-sim`; if a smoke test fails, first check the rail LEDs and the USB port, then the section's pins in `pins.h`; the number needs a method sentence. DEMO: rehearse once against the clock (~15 minutes); the student speaks, you keep time; the video is the safety net, not the plan; the "beyond the baseline" item only if it exists — never suggest it is expected. State the assessment split if asked; anything further is the instructor's.
