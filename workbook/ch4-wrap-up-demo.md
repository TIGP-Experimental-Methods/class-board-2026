...
## Grading
This part of the course is taught, not ranked. Everyone who does the exercises, ships their block and demos it receives full marks. There is no score sheet and no weighting; the feedback you get is on the work itself — in your pull requests, in the ring review, at the demo.

Optional, for the brave: present at the IAMS SAIL Club lunch.

---
**Tutor notes (`/tutor WRAP`, `/tutor DEMO`).** WRAP: environment `esp32s3`, never `-sim`; if a smoke test fails, first check the rail LEDs and the USB port, then the block's pins in `pins.h`; the number needs a method sentence. DEMO: rehearse once against the clock; the student speaks, you time; the video is the fallback, not the plan; item 5 only if it exists — never suggest it is expected, never mention marks.
