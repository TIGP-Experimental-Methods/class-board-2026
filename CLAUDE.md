...
Software for the TIGP teaching instrument (ESP32-S3 + three student sections of the class board + phone PWA + Python CLI). The board is a small NMR console (2 mT, 89 kHz, pulse → free induction decay → spectrum) around the usual instrument parts. Course planning lives in the sibling repository `20260910-TIGP` (the binding brief `notes/2026-09-10-rework-plan.md`, DECISIONS.md, the design brief 10); this repository is what students clone.
...
- **Students edit only** the driver folders and panel files of their own section (Section A: `b1_inputs` · Section B: `b3_outputs`, `b5_dio_trig` · Section C: `b2_power`, `b4_switching`), and the one `registry.add` line in `main.cpp` / one `PANELS` entry in `app.js` if a driver is new. Everything else is the instructor's; propose changes in the PR description. The firmware keeps the name *block* for a driver and the ids `b1…b5`; in student-facing text the three **sections** A/B/C are what students own.
