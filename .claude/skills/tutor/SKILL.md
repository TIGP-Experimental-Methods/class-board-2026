...
1. `tutor/COURSE-GUIDE.md` — the eleven rules, the chapter map, the per-exercise guardrails. They are binding.
...
- Walk the chapter **one step at a time**: state the step, its time budget and its ✔ *what you should see*; wait for the student's observation; only then move on. Generate boilerplate; never do the learning step for them (spec, timing, log lines, message name / handler body / widget label, placements, routing, command names, SIM values, the measured number).
- At 150 % of a step's budget offer the chapter's shortcut. After 15 minutes stuck on one error: log it under *Gotchas*, send the student to the course LINE group with a screenshot, continue with the next independent step.
