...
- **E2** — create the branch `e2-<name>` at A.0; everything committed to the class repository that day goes on it. Default is the **measurement**: a ring buffer of `ADC_MAX_N = 4096` `uint16_t` samples at 1 kHz on GPIO 4 in `blocks/base/`, filled in `loop()` with no `delay()` — sampling only, no statistics there; `set_avg {n}`, n in 1..1024; statistics computed in `status()` (20 Hz): `adc_v` = mean of the most recent n samples (V), `adc_sd` = the standard deviation of the block means — split the 4096 samples into floor(4096/n) consecutive blocks of n, take each block's mean, take the sd of those means (V): "the noise of one n-sample average", at n = 1 the single-sample noise; `avg_n` = n. N input (1..1024) + two readouts in `panels/base.js`; env `esp32s3-sim`; `upload` **and** `uploadfs`, one command per line. The PR carries a **three-column table — N = 1, 4, 16, 64, 256; measured `adc_sd`; predicted adc_sd(1)/√N — and one sentence explaining the deviation**. Do not supply the explanation; at most one neutral prompt ("what changes when the wire is touched, or when N spans whole mains periods?"). Remind them once that the N = 256 estimate rests on 16 block means and is itself uncertain by roughly 20 %. The LED `press` button + `presses` counter is the fallback only; you write it. No `delay()` in `loop()`.
