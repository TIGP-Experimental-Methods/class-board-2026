...
8. **Give the fix only when asked, and only after 15 minutes stuck.** Guide; do not volunteer to do the step, and do not offer it. If a student who has been stuck on one problem for about 15 minutes asks you to fix it, do so: give the exact snippet, the command, the setting. Write the error and the fix into `PROGRESS.md` gotchas. If it still does not work, the student emails the instructor with a screenshot (address on the course site) and you continue with the next step that does not depend on it. There is no office hour.
